...
- 足りないスロットを `gap` として扱う
- gapを確認質問に変換する
- `answer` / `retrieve_more` / `ask` / `block` のプロダクト向け判断に変換する
- 必要なら判断過程をTiDBに残し、あとからSQLで追えるようにする
...
:::message
この記事は、RAGの検索精度を上げる話ではありません。検索できた記憶を「引き継いでよい状態か」で検査し、必要なら追加検索・確認質問・停止へルーティングし、その判断過程をTiDBに残す実装の話です。
:::
...
flowchart TD
    ...
    E --> G
    G --> J{"引き継げる状態か"}
    J -- "yes" --> A["answer: 回答生成へ進む"]
    J -- "more evidence" --> X["retrieve_more: 追加検索"]
    J -- "ask" --> Q["ask: 確認質問"]
    J -- "unsafe" --> B["block: 回答を止める"]
    G -. "optional" .-> T[("TiDB audit store")]
```
...
4. gapごとに確認質問や追加検索queryを作る
5. `answer` / `retrieve_more` / `ask` / `block` に変換する
...
アプリに組み込む場合は、`TransferabilityGate` を回答前に挟み、その結果を `route_transferability_result` でプロダクト向けの挙動に変換します。
...
elif route.action == "retrieve_more":
    print(route.retrieval_queries)
elif route.action == "ask":
    print(route.questions)
else:
    print("Block this answer for now.")
```

`result.transferability_status` は、記憶そのものが `transferable` / `needs_clarification` / `blocked` のどれに見えるかを表します。一方で `route.action` は、実際のアプリが次に何をするかです。ここを分けておくと、検出器の判断を観測しながら、プロダクト側ではまだ回答を止めない、という導入ができます。
...
sequenceDiagram
    ...
    HG->>DB: slot fill / gap / routeを任意保存
    alt transferable
        HG-->>App: OK
        App->>LLM: 最終回答を生成
        LLM-->>User: 回答案
...
実際のRAGに入れるとき、最初から `block` を本番適用するのは怖いです。そこでHandoverGapでは、判定結果と実際のプロダクト挙動を分けています。

| mode | 実際のaction | 用途 |
|---|---|---|
| `shadow` | 常に `answer`。`recommended_action`、gap、質問をログに残す | 既存RAGの挙動を変えずに観測する |
| `soft` | 常に `answer`。警告やレビュー表示に使う | ユーザーにリスクを見せつつ、自動停止はしない |
| `hard` | `answer` / `retrieve_more` / `ask` / `block` を適用する | 評価とレビュー後に高リスク回答を止める |
...
追加検索できるRAGなら、ユーザーに聞く前に不足slotを検索計画へ変換できます。

| retrieval mode | 挙動 |
|---|---|
| `ask_first` | デフォルト。不足slotを確認質問や停止に変換する |
...
回答許可の厳しさは `safety_policy` で切り替えます。デフォルトの `strict` では、権限、顧客説明状況、代替手段、エスカレーション先のような高リスクslotについて、明示的な `evidence_slots` がない限り回答へ戻しません。十分にレビュー済みのslotを呼び出し側で渡せる場合だけ、`balanced` に寄せる想定です。
...
profile YAML にはslotごとの `retrieval_hints` も書けます。`expand_before_ask` では、`preferred_source_types` と `search_terms` が `route.retrieval_queries` に入り、既存retrieverのmetadata filterや検索語ブーストに使えます。
...
HandoverGapのcore runtimeは、TiDBなしで動きます。`demo`、`detect`、`evaluate`、`report`、`profiles validate`、`privacy-check` までは、OpenAI APIキーやTiDB接続情報を要求しません。

そのうえで、TiDBを任意の監査ストアとして使えるようにしています。理由は、Vector検索だけではありません。
...
これらを別々のストアに分けると、「なぜ止めたのか」を追う処理がアプリ側に散らばります。TiDBに寄せると、回答を止めた理由をSQLで監査できる形にできます。
...
ライブのStreamlitデモでOpenAIスロット抽出とTiDB保存まで試す場合は、まとめて次でも入れられます。
...
ローカルでは、100、1,000、10,000件相当まで監査行の増え方も見ています。TiDB には書かず、同じ detector で監査行をメモリ上だけ生成したときの件数と所要時間です。次の表は 2026-06-15 の検証実行例で、行数は再現しやすい一方、p50 / p95 の所要時間は実行環境で変わります。
...
ある実行例では次の結果でした。行数は確認しやすいですが、p50 / p95 はマシンやその時の負荷で変わります。
...
同梱データセット全体についても、監査行が 1 回の評価 run でどれだけ作られるかを確認しています。こちらも所要時間は実行環境で変わります。
...
評価用に、38件の同梱シナリオを用意しています。これは再現性のための架空データであり、本番精度の主張ではありません。
...
各シナリオは、記憶、証拠イベント、profile、task context、gold gap、gold question、unsafe labelを持ちます。スロット入力は `user_provided`、`deterministic_rules`、`optional_llm` の3種類として扱い、評価結果にはどの入力モードかを明示します。
...
Markdownレポートとして残す場合は、次のように出力できます。
...
HandoverGapでは、slot fillingをゲート本体と分けて扱います。

| Mode | 主な用途 | 実行時依存 |
|---|---|---|
| `user_provided` | レビュー済みslot、フォーム、annotation、上流システムの結果 | なし |
| `deterministic_rules` | キーワード、parser、schema、ETLで抽出したslot | なし |
| `optional_llm` | 雑多なメモや検索証拠からの意味的抽出 | 任意のモデルクライアント |

core packageはLLM専用の抽出器ではありません。安全に使うなら、どのslotがどの根拠で埋まったのかを `provided_slots` と `evidence_slots` に分けて渡すのが大事です。
...
# labels.csv の gold_gap_slots / gold_question_slots / unsafe_transfer_label をローカルでレビューして埋める
...
cd handovergap
python -m pip install -e ".[dev,demo,tidb,openai]"
...
- 足りない前提を回答で補完せず、追加検索・確認質問・停止へ変換できる
- `shadow` / `soft` / `hard` で、既存RAGをいきなり止めずに観測から始められる
...
今回の実装で、RAGが返した記憶をprofileごとの必須スロットで検査し、不足している前提を `retrieve_more` / `ask` / `block` に変換し、必要ならその判断過程をTiDBへ残すところまで作りました。評価データはまだ合成中心ですが、adversarial splitで失敗する条件も見えるようにし、TiDB Cloud上では10,000件の生成ワークロードから25,007件の監査行をSQLで追うところまで確認できています。
