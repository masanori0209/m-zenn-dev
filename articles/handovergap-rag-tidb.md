...
`HandoverGap RAG` という、**RAGが返した記憶を「引き継ぐ人がそのまま使ってよい状態か」で検査するゲート**を作りました。正しく検索できた記憶でも、引き継ぐ相手の責任範囲によっては暗黙の前提が足りず、そのまま渡すと危ないことがあります。そこを回答前に検出して、足りない前提を確認質問へ変換します。判断の過程（どのスロットを埋めようとして、どこが不足し、なぜ止めたか）はTiDBに監査ログとして残します。
...
:::message
この記事は、TiDBを使ったRAG実装の完成形というより、「業務の引き継ぎで本当に怖いのは、検索できないことより、足りない前提を補完してしまうことでは？」という仮説を、小さく動かしてみた記録です。
:::

この記事の持ち帰りポイントは4つです。

1. 正しさと引き継ぎ可能性は別に測る
2. 引き継ぐ人の責任範囲ごとに必須スロットを定義する
3. 不足情報をLLMで補完せず、確認質問へ変換する
4. スロット抽出、gap、質問、引き継ぎ判断をTiDBに残し、あとから説明できるようにする
...
既存のRAG評価を否定したいわけではありません。検索関連度や回答正確性は必要です。ただし業務引き継ぎでは、その上に「引き継いでよいか」を見る指標を足さないと、「正しいが後任には危ない記憶」を見逃します。

| 方式 | 見ているもの | 取りこぼしやすいもの |
|---|---|---|
| Naive RAG | 関連する記憶を返す | 引き継ぐ人が安全に使えるか |
| Hybrid RAG | 関連証拠やリスク警告を足す | 引き継ぎプロファイルごとの不足前提 |
| 一般的なContext Engineering | promptや文脈の渡し方を改善する | なぜ回答を止めたかの監査記録 |
| HandoverGap RAG | 回答前に必須スロットを検査する | 評価データの独立性と過剰質問の扱い |

## HandoverGap RAG の考え方

HandoverGap RAGでは、記憶をそのまま回答に使わず、まず「引き継ぐ人が使ってよい状態か」を見ます。
...
| `authority` | 引き継ぐ人が顧客向けに約束・回答してよい範囲 |
...
つまり、既存RAGを置き換えるのではなく、RAGが返した記憶を「引き継ぐ人が安全に使える形か？」で検査します。TiDBには、その判断過程を監査ログとして残します。
...
先に、この設計でTiDBが効いてくるところを書いておきます。

HandoverGapで一番見たいのは「なぜこの引き継ぎを止めたのか」です。その答えは、後段の監査クエリに集約されます。

このクエリでは、`transfer_assessments` の関係結合（SQL）、スロットを埋めるための証拠検索（Vector Search）、顧客名やIssue IDの一致（Full-text Search）、Slack/Issue/議事録のメタデータ（JSON）を、同じストアの中で突き合わせています。

これらを別々のDBに分けると、判断過程の突き合わせがアプリ側のコードに散ってしまいます。TiDBを選んだのは、「なぜ止めたか」を1つのストアから追える形にしたかったからです。
...
ライブ検証ではTiDB CloudのDeveloper Tierに接続し、同梱schemaの作成、検証用memoryの保存、スロット抽出試行、context gap、transfer assessment、評価結果の保存まで確認しました。
...
blockedになった引き継ぎで多かった不足スロットは次の通りです。
...
これはTiDBクエリレイテンシの主張ではありません。ここで測っているのは、HandoverGapがTiDBへ保存・照会する監査ワークロードのサイズと、ローカルでの監査行生成時間です。TiDB上では、この行たちを先ほどのSQLで横断して追います。
...
これは負荷試験ではなく、TiDB Cloud Developer Tierでの小さな実測です。ただ、少なくとも「監査SQLは机上の設計だけではなく、TiDB上に行を保存してJOINで追える」ことは確認できました。ここはTiDBを使った記事として、曖昧にせず数字を出しておきたいところでした。
...
| False Clarification Rate | 低いほど良い | 安全な記憶に不要な確認質問を出した割合 | 高いと、引き継ぐ人に余計な確認負荷をかける |
...
| sanitized | 匿名化された業務メモ風の文脈で見る | 実データに近い書きぶりでの小さな評価 |
| holdout | slot fillingの揺れを見る | LLMや抽出方針への感度を見る |

まず、`provided_slots` と `gold_gaps` の構造的な一致を崩した adversarial split を用意しました。ここでは、あえてHandoverGapが間違えやすいケースを入れています。安全なのにスロット抽出が一部欠けるケースと、危険なのにスロットが埋まったように見えるケースを混ぜています。
...
もう一つ、実データは使わずに、より現場の記録に近い書きぶりへ寄せた `sanitized` split も追加しました。これは実在の会社、社員、顧客、チケット、商談データを含みません。ただし、CRMメモ、インシデントタイムライン、Runbook、リリースチェックリスト、商談レビューのような業務記録を想定して書いています。
...
もちろん、これも本物の社内データではありません。ですが、mini datasetのような短い作例だけでなく、業務メモ風のケースでも、監査行と評価結果を同じ形で出せるようになりました。検証用データ中心という弱点は残りますが、少なくとも「短い作り話20件だけ」の状態からは一歩進んでいます。
...
しかし、引き継ぎでは「知らないことを知らないまま扱う」ほうが重要な場面があります。ここを回答文のうまさで隠してしまうと、引き継ぐ人は安心して間違えます。
...
TiDB Cloudに接続して監査SQLを実際に動かす場合は、TiDB接続情報を設定したうえで次を実行します。
...
この試作には、はっきりした限界があります。特に重要なのは、評価データが **実在の社内ログではない** ことです。
...
- TiDBの監査SQLは小さな実測であり、大規模負荷・障害試験ではない

次にやるなら、もう少し実際の引き継ぎに近いデータで試したいです。特に、どの情報を「足りない前提」と見るかは主観が入りやすいので、実装者とは別の人がラベルを付ける形にしたいところです。TiDBの監査SQLも、今回は小さなデータでの確認に留まっているので、行数を増やしたときにどのくらい追いやすいかは見てみたいです。

## まとめ

今回作ったものは、RAGの評価を「正しく答えられるか」から、「引き継ぐ人に渡してよい状態か」まで広げる小さな実験です。
