...
- 行数レポート（`scripts/count-lines.sh`）付き。測定の透明性は残したまま、FW として触れる
- 「性能」や「本番向き」「Django クローン」は主張しません。行数測定が出発点、ミニ FW が到着点です
...
![投稿一覧（indexed file）](/images/cobol-webfw-posts.png)

![run-all.sh の実行例](/images/cobol-webfw-run-all.png)
...
GnuCOBOL が `stdout` に `Content-Type` と HTML を出す CGI は、**接続方式が CGI で、レンダリングモデルが SSR** です。Next.js も Django も、ここでは同じ側（サーバーで HTML を組む）に立ちます。違いはその**上に載せる層**です。今回の cobol-webfw は、測り始めは Next.js 寄り、載せ終わると Django 寄り——というズレが面白かった、という話です。
...
<!-- evidence: command="./scripts/count-lines.sh"; log="reports/line-count.txt" -->
...
<!-- evidence: command="./scripts/build.sh"; log="src/config.cbl" -->
...
<!-- evidence: command="BASE_URL=http://127.0.0.1:8080 ./scripts/run-all.sh"; log="cobol-webfw/reports/run-all.log" -->
...
**物差しは Next.js、できあがった形は Django 寄り。** 行数を数えるつもりが、MTV のミニ FW を組み立てていた——それが cobol-webfw の話です。
