...
Rust（Tauri v2）でマシンの数値を取って、画面は React です。見た目は今どきのダッシュボードっぽくせず、古い Mac のウィンドウに寄せています。
...
手元の Mac（Apple Silicon）だと、`npm run tauri dev` で起動したときに温度も出ました。ブラウザで Vite を開いただけだと本物の数値には届かないので、確認はネイティブ起動でやっています。
...
Rust でデスクトップアプリを出したくて、ついでに CPU 温度やメモリも見たいな、と思ったのがきっかけです。そうなると、だいたいシステムモニタになります。

CPU とディスクはわりと素直でした。温度も、少なくともこの Mac では `sysinfo` で取れました。

困ったのはメモリのほうです。used が高くて available が 0、swap が厚い。この並びをどう読むか、が地味にむずかしいです。
...
`http://localhost:1420` は UI の確認用です。温度や実メモリを見るなら、Tauri で起動したほうがいいです。
...
手元では `PMU tdie*` や `gas gauge battery` で ℃ が出ました。全部出すと `tdev` みたいなチャンネルも混ざって、マイナスの温度が出たりします。なのでラベルで絞っています。
...
今回のスクショは `available` 寄りの状態です。ただ、ラベルがそのまま「CPU」ではないことが多いですし、別のマシンだと空になるかもしれません。

<!-- evidence: command="npm run tauri dev"; log="images/sysgauge-dashboard.png" -->

なので「温度対応した」というより、**この Mac では取れた**、くらいの温度感です。取れないときは `No sensors` と出して、アプリ自体は止めないようにしています。

フロントからは、だいたいこんな感じで Rust を呼びます。
...
macOS の `sysinfo` だと、available はざっくりこんな計算です。

```text
available ≈ (free + inactive + purgeable - compressor) × page_size
```

compressor（圧縮メモリ）が大きいと引き算がマイナスになって、`saturating_sub` で **0** になります。Activity Monitor や `memory_pressure` の「余裕」とは定義が違うので、available だけを信じすぎないほうがよさそうです。

<!-- evidence: command="npm run tauri dev"; log="images/sysgauge-dashboard.png" -->

なので UI には説明文を置かず、4 つの数字を同じ段に並べるだけにしました。
...
今風のカード UI にすると、どうしても Web を箱に入れた感じになります。なので、古い Mac に寄せました。

- Platinum っぽいタイトルバーとベベルボタン
- Controls 風の group box
- 斜線のプログレスバー
- グレーのドットデスクトップ
- だいたい 840×520 で、ほぼスクロールしない

fieldset はただの枠線ではなく、外枠と内側のハイライトを重ねて、昔のコントロールパネルっぽくしています。
...
次にやるなら、Windows 向けの経路を feature flag で足すのが自然かなと思っています。
...
数字が出ることと、数字の読み方は別物でした。温度は取れたけど、そこが残った、という感じです。
