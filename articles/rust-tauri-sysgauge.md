...
数値そのものは共通 API で取れます。ただし **同じフィールド名でも、頭の中の「使用率」と一致するとは限らない**。ここを本文で先に言っておかないと、スクショの印象だけで誤解されます。
...
### 1. CPU・ディスクは共通 API で先に埋められる

`sysinfo` だけで、ダッシュボードの主戦場は埋まります。先にここを固めると、アプリとして先に「使える」状態になります。

### 2. 温度は、少なくともこの Mac では `Components` で出た

「Apple Silicon では絶対無理」と言い切る必要はなかった、というのが今回の実測です。ただしラベルと値の信頼度は別問題です。

### 3. メモリは値より「どの値を主に見せるか」がプロダクト判断

sysgauge は used を大きく出し、available / free / swap を同じ段に並べました。説明文は UI に置きません。

### 4. 確認はブラウザではなく Tauri でやる

Vite の URL は UI の下書き確認には便利です。温度や実メモリはネイティブ側で見ないと話がずれます。
